 ...
 */
public object DecodeHintManager {
    ...
    fun parseDecodeHints(inputUri: Uri): EnumMap<DecodeHintType, Any>? {
            ...
            if (hintType.valueType == IntArray::class.java) {
                ...
                for (i in values.indices) {
                    try {
                        ...
                        Log.w(
                            TAG,
                            "Skipping array of integers hint $hintType due to invalid numeric value"
                        )
                        array = null
                        break
                    }
    ...
    fun parseDecodeHints(intent: Intent): EnumMap<DecodeHintType, Any>? {
            ...
            if (extras.containsKey(hintName)) {
                if (hintType.valueType == Void::class.java) {
                    ...
                    if (hintType.valueType.isInstance(hintData)) {
                        ...
                        Log.w(
                            TAG,
                            "Ignoring hint " + hintType + " because it is not a " + hintType.valueType
                        )
                    }
                }
            }
