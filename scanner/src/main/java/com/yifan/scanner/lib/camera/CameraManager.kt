...
import com.yifan.scanner.lib.camera.open.OpenCameraInterface
...
class CameraManager(private val context: Context) {
    ...
    fun setManualFramingRect(width: Int, height: Int) {
        ...
        if (initialized) {
            ...
            val topOffset = (screenResolution.y - h) / 2
            framingRect = Rect(leftOffset, topOffset, leftOffset + w, topOffset + h)
...
// Go ahead and assume it's YUV rather than die.
    ...
    companion object {
        ...
        private fun findDesiredDimensionInRange(resolution: Int, hardMin: Int, hardMax: Int): Int {
            ...
            return if (dim < hardMin) {
                hardMin
            } else Math.min(dim, hardMax)
        }
    }
